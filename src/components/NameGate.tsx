...
const FIELD =
  "w-full rounded-md border border-grey-30 bg-card px-2.5 py-1.5 text-sm text-grey-90 focus:border-rmit-blue-interactive focus:outline-2 focus:outline-offset-0 focus:outline-rmit-blue-interactive";
...
export function NameGate({ onDone }: { onDone: (user: MapUser) => void }) {
  ...
  const resolvedPortfolio = portfolio === OTHER ? otherText.trim() : portfolio;
  const ready = Boolean(name.trim() && resolvedPortfolio);

  const submit = async () => {
    if (!ready || saving) return;
    setSaving(true);
    onDone(await registerUser(name, resolvedPortfolio));
  };
    ...
    >
      <div className="w-full max-w-sm rounded-lg border border-grey-30 bg-card p-6 shadow-xl">
        ...
        >
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-grey-70">Portfolio</span>
            <select value={portfolio} onChange={(e) => setPortfolio(e.target.value)} className={FIELD}>
          ...
          {portfolio === OTHER && (
            <label className="flex flex-col gap-1">
              <span className="text-xs text-grey-70">Your portfolio or team</span>
              <input
                type="text"
                value={otherText}
